...
#endif
#endif
    ! Defaults for the neutral flux limiter acting on Dnn.
    phys%fluxlim_nn_gamma = 1.d0
    phys%fluxlim_nn_alpha = 1.d0
    phys%fluxlim_nn_omega = 0.2d0
#ifdef EXPANDEDCX
#ifdef AMJUELCX
...
#ifdef NEUTRAL
  SUBROUTINE apply_neutral_flux_limiter_to_diffusion(q, up, d_iso, deff_prev, deff_new)
    REAL*8, INTENT(IN)    :: q(:, :), up(:, :)
    REAL*8, INTENT(INOUT) :: d_iso(:, :, :)
    REAL*8, INTENT(IN)    :: deff_prev(:)
    REAL*8, INTENT(OUT)   :: deff_new(:)
    REAL*8                :: qpr(simpar%Ndim,simpar%Neq)
    REAL*8                :: dnn_loc, grad_nn, nn_loc, csi_loc, ratio, gamma_loc, alpha_loc
    INTEGER               :: ig
    REAL*8, PARAMETER     :: nn_min = 1.d-10, csi_min = 1.d-12, gamma_min = 1.d-12

    gamma_loc = MAX(phys%fluxlim_nn_gamma, gamma_min)
    alpha_loc = MAX(phys%fluxlim_nn_alpha, gamma_min)
    deff_new = deff_prev
    DO ig = 1, SIZE(q,1)
      ...
      deff_new(ig) = dnn_loc*(1.d0 + ratio**gamma_loc)**(-1.d0/gamma_loc)
      IF (deff_prev(ig) > 0.d0) THEN
        deff_new(ig) = phys%fluxlim_nn_omega*deff_new(ig) + (1.d0-phys%fluxlim_nn_omega)*deff_prev(ig)
      END IF
      d_iso(5,5,ig) = deff_new(ig)
    END DO
