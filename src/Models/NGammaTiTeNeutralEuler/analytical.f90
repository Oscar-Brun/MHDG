...
CONTAINS
  ...
  SUBROUTINE analytical_solution(iel,x, y, u)
    ...
    CASE (50:59)
       ...
       ENDIF
#ifdef NEUTRALEULER
       if (phys%idx_rhon_pv > 0) up(:, phys%idx_rhon_pv) = 1.e-8
       if (phys%idx_unx_pv > 0)  up(:, phys%idx_unx_pv)  = 0.d0
       if (phys%idx_uny_pv > 0)  up(:, phys%idx_uny_pv)  = 0.d0
       if (phys%idx_k_pv > 0)    up(:, phys%idx_k_pv)    = 1.8e-5
#elif defined(NEUTRALGAMMA)
       if (phys%idx_rhon_pv > 0) up(:, phys%idx_rhon_pv) = 1.e-8
       if (phys%idx_un_pv > 0)   up(:, phys%idx_un_pv)   = 0.d0
       if (phys%idx_k_pv > 0)    up(:, phys%idx_k_pv)    = 1.8e-5
#else
#ifdef NEUTRAL
       if (phys%idx_rhon_pv > 0) up(:, phys%idx_rhon_pv) = 1.e-8
    ...
    CASE (60:64)
       ...
       if (phys%idx_unx_pv > 0)  up(:, phys%idx_unx_pv)  = 0.d0
       if (phys%idx_uny_pv > 0)  up(:, phys%idx_uny_pv)  = 0.d0
       if (phys%idx_k_pv > 0)    up(:, phys%idx_k_pv)    = 1.e-5
#elif defined(NEUTRALGAMMA)
       if (phys%idx_rhon_pv > 0) up(:, phys%idx_rhon_pv) = 1.e-8
       if (phys%idx_un_pv > 0)   up(:, phys%idx_un_pv)   = 0.d0
       if (phys%idx_k_pv > 0)    up(:, phys%idx_k_pv)    = 1.e-5
