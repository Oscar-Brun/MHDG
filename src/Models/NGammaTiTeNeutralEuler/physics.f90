...
CONTAINS
  ...
  SUBROUTINE set_model_layout()
    ...
    phys%idx_gammanx_eq = phys%idx_rhon_eq + 1
    phys%idx_gammany_eq = phys%idx_rhon_eq + 2
    phys%idx_gamman_eq = phys%idx_gammanx_eq
    phys%idx_unx_pv = phys%idx_rhon_pv + 1
    phys%idx_uny_pv = phys%idx_rhon_pv + 2
    phys%idx_un_pv = phys%idx_unx_pv
    phys%Neq = phys%idx_gammany_eq
    phys%npv = phys%idx_uny_pv
    simpar%model = 'N-Gamma-Ti-Te-NeutralEuler'
#endif
...
#ifdef NEUTRALEULER
    simpar%model = 'N-Gamma-Ti-Te-NeutralEuler-k'
...
#ifdef NEUTRALEULER
    if (phys%idx_unx_pv > 0) phys%phyVarNam(phys%idx_unx_pv)= "unx"
    if (phys%idx_uny_pv > 0) phys%phyVarNam(phys%idx_uny_pv)= "uny"
#endif
#ifdef NEUTRALGAMMA
...
#ifdef NEUTRALEULER
    if (phys%idx_gammanx_eq > 0) phys%conVarNam(phys%idx_gammanx_eq) = "Gammanx"
    if (phys%idx_gammany_eq > 0) phys%conVarNam(phys%idx_gammany_eq) = "Gammany"
#endif
#ifdef NEUTRALGAMMA
...
#ifdef NEUTRALEULER
    if (phys%idx_unx_pv > 0) simpar%physvar_refval(phys%idx_unx_pv) = simpar%refval_speed
    if (phys%idx_uny_pv > 0) simpar%physvar_refval(phys%idx_uny_pv) = simpar%refval_speed
#endif
#ifdef NEUTRALGAMMA
...
#ifdef NEUTRALEULER
    if (phys%idx_gammanx_eq > 0) simpar%consvar_refval(phys%idx_gammanx_eq) = simpar%refval_momentum
    if (phys%idx_gammany_eq > 0) simpar%consvar_refval(phys%idx_gammany_eq) = simpar%refval_momentum
#endif
#ifdef NEUTRALGAMMA
...
#ifdef NEUTRALEULER
    if (phys%idx_gammanx_eq > 0 .and. phys%idx_unx_pv > 0 .and. phys%idx_rhon_pv > 0) then
      ua(:,phys%idx_gammanx_eq) = ABS(MAX(up(:,phys%idx_rhon_pv), 1.d-7))*up(:,phys%idx_unx_pv)
    end if
    if (phys%idx_gammany_eq > 0 .and. phys%idx_uny_pv > 0 .and. phys%idx_rhon_pv > 0) then
      ua(:,phys%idx_gammany_eq) = ABS(MAX(up(:,phys%idx_rhon_pv), 1.d-7))*up(:,phys%idx_uny_pv)
    end if
#endif
#ifdef NEUTRALGAMMA
...
#ifdef NEUTRALEULER
    if (phys%idx_gammanx_eq > 0 .and. phys%idx_unx_pv > 0) then
      up(:,phys%idx_unx_pv) = ua(:,phys%idx_gammanx_eq)/ABS(MAX(U5, 1.d-7))
    end if
    if (phys%idx_gammany_eq > 0 .and. phys%idx_uny_pv > 0) then
      up(:,phys%idx_uny_pv) = ua(:,phys%idx_gammany_eq)/ABS(MAX(U5, 1.d-7))
    end if
#endif
#ifdef NEUTRALGAMMA
...
!  Ay = Ay/simpar%refval_speed
 ...
 SUBROUTINE interpolate_1D_diff(rho,diff_1D)
  ...
  SUBROUTINE setLocalDiff(xy, u, d_iso, d_ani, q_cyl)
    ...
    d_ani(phys%idx_gammanx_eq, phys%idx_gammanx_eq, :) = 0.d0
    d_ani(phys%idx_gammany_eq, phys%idx_gammany_eq, :) = 0.d0
#else
...
#ifdef NEUTRALEULER
  SUBROUTINE compute_fGammaN(U,b,fGammaN)
    ...
    gamman_par = U(ignx)*b(1) + U(igny)*b(2)
    fGammaN = U(1)*gamman_par
  ENDSUBROUTINE compute_fGammaN


  SUBROUTINE compute_dfGammaN_dU(U,b,res)
    REAL*8, INTENT(IN)  :: U(:), b(:)
    REAL*8, INTENT(OUT) :: res(:)
    INTEGER             :: ignx, igny
...
! These routines use AMUJUEL splines
  ...
  SUBROUTINE compute_fEiN(U,fEiN)
    ...
    fEiN = 0.5d0*U(1)*(U(ignx)**2 + U(igny)**2)/Unn
  ...
  SUBROUTINE compute_dfEiN_dU(U,res)
    ...
    res(1) = (U(ignx)**2 + U(igny)**2)/Unn
    IF (U(inn) >= tol) res(inn) = -U(1)*((U(ignx)**2 + U(igny)**2)/Unn**2)
    res(ignx) = 2.d0*U(1)*U(ignx)/Unn
    res(igny) = 2.d0*U(1)*U(igny)/Unn
...
!#endif
    ...
    if (phys%idx_rhon_eq > 0) then
      block
        real*8 :: un_n, cs_n, tau_euler
        un_n = up(phys%idx_unx_pv)*n(1) + up(phys%idx_uny_pv)*n(2)
        cs_n = SQRT(MAX(up(7), 1.d-10))
        tau_euler = MAX(numer%tau(phys%idx_rhon_eq), ABS(un_n) + cs_n)
        tau(phys%idx_rhon_eq, phys%idx_rhon_eq) = tau_euler
        tau(phys%idx_gammanx_eq, phys%idx_gammanx_eq) = tau_euler
        tau(phys%idx_gammany_eq, phys%idx_gammany_eq) = tau_euler
      end block
    end if
#endif
#ifdef NEUTRALGAMMA
